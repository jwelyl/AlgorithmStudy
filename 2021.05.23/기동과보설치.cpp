...
bool checkPillar(int x, int y) {
    ...
    if((Bar[x - 1][y] && x > 0) || Bar[x][y]) return true; 
    return false;
}

bool checkBar(int x, int y) {
    if(Pillar[x][y - 1] || Pillar[x + 1][y - 1]) return true;
    if(Bar[x + 1][y] && Bar[x - 1][y] && x > 0) return true;
    return false;
}
...
vector<vector<int>> solution(int n, vector<vector<int>> build_frame) {
    ...
    for(auto build : build_frame) {
        ...
        if(type == 0) { // 기둥
            if(cmd == 1) { // 설치
                if(checkPillar(x, y)) Pillar[x][y] = true;
            }
            else { // 삭제
                Pillar[x][y] = false;
                if(isOk(x, y) == false) Pillar[x][y] = true;
            }
        }
        else {  // 보
            if(cmd == 1) { // 설치
                if(checkBar(x, y)) Bar[x][y] = true;
            }
            else {
                Bar[x][y] = false;
                if(isOk(x, y) == false) Bar[x][y] = true;
            }
        }
    }
